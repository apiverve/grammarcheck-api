...
class GrammarcheckValidationException implements Exception {
  final List<String> errors;

  GrammarcheckValidationException(this.errors);

  @override
  String toString() => 'GrammarcheckValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/grammarcheck?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The text you want to check for grammatical and spelling errors
class GrammarcheckClient {
  final String apiKey;
  ...
  GrammarcheckClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/grammarcheck',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [GrammarcheckValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw GrammarcheckValidationException(errors);
    }
  }

  /// Execute a request to the Grammar Check API.
  ///
  /// Parameters are validated before sending the request.
  Future<GrammarcheckResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw GrammarcheckException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return GrammarcheckResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw GrammarcheckException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw GrammarcheckException('Resource not found');
      } else {
        throw GrammarcheckException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is GrammarcheckException) rethrow;
      throw GrammarcheckException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Grammar Check API client.
class GrammarcheckException implements Exception {
  final String message;

  GrammarcheckException(this.message);

  @override
  String toString() => 'GrammarcheckException: $message';
}
